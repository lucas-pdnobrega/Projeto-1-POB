...
public class TelaCarne {
	...
	private void initialize() {
		...
		button_1 = new JButton("Criar nova carne");
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					Fachada.cadastrarCarne(nome, preco);
					label.setText("Carne criada: " + nome);
					listagem();
				}
				catch(Exception ex) {
			...
			public void actionPerformed(ActionEvent e) {
				try{
					if (table.getSelectedRow() >= 0){	
						...
						Fachada.excluirCarne(nome);
						label.setText("Carne apagada" );
						listagem();
					}
					else
	...
	public void listagem() {
		try{
			List<Carne> lista = Fachada.listarCarnes();
			...
			for(Carne c : lista) {
				model.addRow(new Object[]{c.getNome(), c.getPreco()} );
			}
